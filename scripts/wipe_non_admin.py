...
# ruff: noqa: E402
# Imports from ``app`` must follow the backend path bootstrap below.
...
# Every user-scoped table keys on the stable users.id integer FK. The ``users``
...
def _count_rows_per_table(db, admin_username: str) -> dict[str, int]:
    ...
    """
    admin_pk = db.execute(
        text("SELECT id FROM users WHERE username = :auname"),
    ...
    queries: list[tuple[str, str, dict]] = [
        (
            ...
            "ON cm.conversation_id = cs.id WHERE cs.user_id != :apk",
            {"apk": admin_pk},
        ),
        (
            "interview_qa",
            "SELECT COUNT(*) FROM interview_qa iq JOIN interview_records ir "
            "ON iq.record_id = ir.id WHERE ir.user_id != :apk",
            {"apk": admin_pk},
        ),
    ]
    queries += [
...
def _non_admin_doc_ids(db, admin_username: str) -> list[str]:
    ...
    rows = db.execute(
        text("SELECT id FROM knowledge_documents WHERE user_id != :apk"),
        {"apk": admin_pk},
    ).fetchall()
...
def _delete_milvus_vectors(doc_ids: list[str], dry_run: bool) -> dict[str, int]:
    ...
    port = (
        milvus_uri.rsplit(":", 1)[-1]
        if ":" in milvus_uri.replace("://", "")
        else "19530"
    )
    ...
    for name in MILVUS_COLLECTIONS:
        ...
        for i in range(0, len(doc_ids), chunk_size):
            ...
            try:
                ...
                count_res = c.query(
                    expr=expr, output_fields=["doc_id"], limit=chunk_size
                )
...
def _delete_postgres_rows(db, admin_username: str, dry_run: bool) -> None:
    ...
    deletes: list[tuple[str, str, dict]] = [
        (
            ...
            "(SELECT id FROM interview_records WHERE user_id != :apk)",
            {"apk": admin_pk},
        ),
        (
            "conversation_messages",
            "DELETE FROM conversation_messages WHERE conversation_id IN "
            "(SELECT id FROM conversations WHERE user_id != :apk)",
            {"apk": admin_pk},
        ),
    ]
    deletes += [
    ...
    db.execute(
        text("DELETE FROM users WHERE username != :auname"), {"auname": admin_username}
    )
    db.commit()
...
def main() -> None:
    ...
    if not args.dry_run and not args.yes:
        sys.exit(
            "Refusing to run destructively without --yes. Re-run with --dry-run first."
        )
    ...
    try:
        ...
        print(
            f"  Wipe:    {len(others)} other user(s): "
