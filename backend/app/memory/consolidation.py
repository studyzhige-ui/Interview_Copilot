...
from app.memory import lifecycle as memory
from app.memory.projections import _workspace
from app.memory.projections import invalidate_workspace
from app.memory.projections import suppress_sources
from app.memory.prompts import EXTRACT
from app.memory.prompts import CONSOLIDATE
from app.memory.prompts import SELECT
...
async def extract_turn(turn_id: str) -> int | None:
    ...
    try:
        ...
        with scope(source.user_pk, f"memory-extract:{turn_id}:{source_hash}"):
...
async def consolidate_user(user_id: int) -> int:
    ...
    try:
        ...
        with scope(user_id, f"memory-consolidate:{input_hash}"):
