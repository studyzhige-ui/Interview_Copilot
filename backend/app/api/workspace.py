...
def overview(
    ...
    for session in sessions:
        ...
        recent.append(
            RecentWork(
                ...
                status=(turn.status if turn.status in statuses else "unknown")
                if turn
                else "not_started",
    ...
    return WorkspaceOverview(
        has_resume=db.query(Artifact.id)
        .filter(
        ...
        .first()
        is not None,
        active_opportunities=db.query(JobOpportunity)
        .filter(
        ...
        .count(),
        open_actions=db.query(NextAction)
        .filter(
