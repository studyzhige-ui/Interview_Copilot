...
from app.core.execution_errors import (
    ModelOutcomeUnknownError as ModelOutcomeUnknownError,
)
from app.core.execution_errors import (
    ModelDispatchConflictError as ModelDispatchConflictError,
    ModelStreamCapacityError as ModelStreamCapacityError,
)
from app.core.provider_streams import close_provider_stream
from app.core.provider_usage import accumulate, ledger_units, logical_tokens
from app.core.model_request_identity import request_fingerprint
from app.core.context_budget import ContextCapacityError
from app.usage import service as model_budget_service
...
def start_model_dispatch(
    ...
    *,
    ...
    usage_units: dict[str, int] | None = None,
) -> AgentModelDispatch:
    turn = (
    ...
    *,
    ...
    try:
        ...
        if row.status != "running":
            return
        observed_tokens = logical_tokens(usage or {})
        ...
        model_budget_service.settle(
            ...
            observed_units=ledger_units(usage or {}),
        )
        row.status = status
    ...
    try:
        while True:
            ...
            if observed is not None:
                ...
                try:
                    accumulate(usage, observed)
                except ValueError as exc:
                    raise ModelStreamCapacityError("invalid_provider_usage") from exc
            yield chunk
        await flush()
