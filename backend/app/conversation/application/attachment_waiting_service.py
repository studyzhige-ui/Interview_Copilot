...
def _resume_candidate(
    db, *, turn_id: str, user_id: int, actions: ResumeActions
) -> bool:
    if not _turn_parsing_is_ready(db, turn_id):
        return False

    generation = actions.resume(
        db,
        turn_id,
        user_id,
...
def wake_attachment_turn_if_terminal(turn_id: str, *, actions: ResumeActions) -> bool:
...
def recover_terminal_attachment_turns(
    *, actions: ResumeActions, limit: int = 100
) -> list[str]:
    ...
    try:
        ...
        for turn_id, user_id in candidates:
            if _resume_candidate(db, turn_id=turn_id, user_id=user_id, actions=actions):
...
def wake_attachment_turns_for_projection(
    document_id: str, *, actions: ResumeActions
) -> list[str]:
