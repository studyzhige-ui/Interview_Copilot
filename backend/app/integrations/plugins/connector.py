...
from app.integrations.plugins.credentials import EncryptedFilePluginCredentialStore
from app.integrations.plugins.credentials import PluginCredentialConflictError
from app.integrations.plugins.credentials import PluginCredentialGrant
from app.integrations.plugins.credentials import PluginCredentialNotFoundError
from app.integrations.plugins.credentials import PluginCredentialStore
from app.integrations.plugins.credentials import PluginCredentialStoreError
from app.integrations.plugins.credentials import PluginCredentialStoreUnavailableError
...
class OAuthPluginConnector:
    ...
    async def _authorized_json(
        ...
        from app.usage.external import request

        current = await self._fresh_grant(grant)
        headers = dict(kwargs.pop("headers", {}))
        headers["Authorization"] = f"Bearer {current.access_token}"
        response = await request(
            lambda: self._request(method, url, headers=headers, **kwargs),
            owner=grant.user_pk,
            provider=self.provider,
            operation="resource",
            content={"method": method, "url": url, "payload": kwargs},
        )
