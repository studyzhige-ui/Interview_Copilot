...
async def test_generate_report_empty_when_no_docstore():
    ...
    with patch("app.services.analytics_service.os.path.exists", return_value=False):
...
async def test_generate_report_empty_when_no_personal_memories():
    ...
    with patch("app.services.analytics_service.os.path.exists", return_value=True), \
         patch("app.services.analytics_service.SimpleDocumentStore.from_persist_dir", return_value=mock_docstore):
        from app.services.analytics_service import generate_comprehensive_report
        result = await generate_comprehensive_report(limit=20, user_id="u1")
        assert result["status"] == "empty"
...
async def test_generate_report_successful_json_parse():
    ...
    with patch("app.services.analytics_service.os.path.exists", return_value=True), \
         patch("app.services.analytics_service.SimpleDocumentStore.from_persist_dir", return_value=mock_docstore), \
         patch("app.services.analytics_service.agent_fast_llm") as llm_mock:
