...
def test_resume_parse_and_store(monkeypatch, resume_db_session):
    ...
    rows = (
        resume_db_session.query(ResumeSection)
        .filter(ResumeSection.resume_id == "rsm_1")
        .all()
    )
    assert len(rows) == 3
            ...
            return FakeResponse(
                json.dumps(
                    [
                        {
                            "section_type": "summary",
                            "title": f"Version {self.call_count}",
                            "content": "Content",
                        }
                    ]
                )
...
def test_persist_handles_invalid_section_type(monkeypatch, resume_db_session):
    ...
    sections = asyncio.run(
        module.ResumeService().extract_and_store(
