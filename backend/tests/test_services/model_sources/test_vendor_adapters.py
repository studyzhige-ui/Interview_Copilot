...
def test_chat_filter(vendor, bare_id, kept):
    ...
    if spec.chat_filter is None:
        ...
        assert kept is True, (
            f"{vendor} has no filter — test row should expect kept=True"
        )
    else:
...
async def test_fetch_anthropic_uses_xapi_and_version(monkeypatch):
    ...
    await fetch_one_vendor(
        get_spec("anthropic"), "https://api.anthropic.com/v1", "sk-ant-x"
    )
...
async def test_fetch_rejects_top_level_non_dict(monkeypatch):
    _install_transport(
        monkeypatch, lambda r: httpx.Response(200, json=["not", "a", "dict"])
    )
...
async def test_fetch_rejects_missing_response_key(monkeypatch):
    _install_transport(
        monkeypatch, lambda r: httpx.Response(200, json={"wrong_key": []})
    )
