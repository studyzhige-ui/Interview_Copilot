"""Nightly memory-consolidation gate tests.
    ...
    try:
        ...
        for i in range(messages):
            m = ConversationMessage(
                conversation_id=session_id, seq=i, role="User", content=f"m{i}"
            )
    ...
    try:
        ...
        db.add(
            Conversation(
                ...
                subject_id="ir_active",
            )
        )
        db.add(
            ConversationMessage(
                conversation_id="s_active",
                seq=0,
                role="User",
                content="hi",
        ...
        db.add(
            Conversation(
                ...
                type="debrief",
                subject_type="interview_record",
                subject_id="ir_settled",
            )
        )
        db.add(
            ConversationMessage(
                conversation_id="s_settled",
                seq=0,
                role="User",
                content="hi",
