...
def test_delete_queues_outbox_when_milvus_fails(db_session, monkeypatch):
    ...
    doc = (
        ...
        .filter(KnowledgeDocument.id == "kdoc_f")
        .first()
    )
    ...
    assert (
        ...
        .filter(DocumentChunk.document_id == "kdoc_f")
        .count()
        == 0
    )
...
def test_enqueue_then_drain_runs_registered_handler(db_session, monkeypatch):
    ...
    job = (
        db_session.query(OutboxJob)
        .filter(OutboxJob.aggregate_id == "kdoc_drain")
        .first()
    )
...
def test_upsert_handler_rebuilds_and_marks_ready(db_session, monkeypatch):
    ...
    doc = (
        db_session.query(KnowledgeDocument)
        .filter(KnowledgeDocument.id == "kdoc_u")
        .first()
    )
...
def test_upsert_handler_nonfinal_failure_stays_processing(db_session, monkeypatch):
    ...
    doc = (
        db_session.query(KnowledgeDocument)
        .filter(KnowledgeDocument.id == "kdoc_u2")
        .first()
    )
...
def test_upsert_handler_final_failure_marks_failed(db_session, monkeypatch):
    ...
    doc = (
        db_session.query(KnowledgeDocument)
        .filter(KnowledgeDocument.id == "kdoc_u3")
        .first()
    )
...
def test_upsert_handler_does_not_resurrect_deleting_doc(db_session, monkeypatch):
    ...
    doc = (
        db_session.query(KnowledgeDocument)
        .filter(KnowledgeDocument.id == "kdoc_del")
        .first()
    )
...
def test_upsert_drain_persistent_failure_ends_dead_and_doc_failed(
    ...
    for run in range(1, 6):
        ...
        if run < 5:
            assert _upsert_job_row(db_session, "kdoc_e2e").status == "failed", (
                f"run {run}"
            )
