...
BASE = '62a1520e2a2b6085b5830a34300b7278f782925a'
DIGEST = '00bfbcf7bfc69291f38e96905c8f49529e82c39b48ce2c15b4ceb438a4abb334'
...
def apply():
    ...
    encoded = ''.join((ROOT / f'.github/_stage/payload{i}.txt').read_text() for i in range(3))
    decoder = lzma.LZMADecompressor(memlimit=64 * 1024 * 1024)
    raw = decoder.decompress(base64.b64decode(encoded, validate=True), max_length=74755)
    if len(raw) != 74754 or not decoder.eof or decoder.unused_data or hashlib.sha256(raw).hexdigest() != DIGEST:
        raise SystemExit('payload_integrity')
    value = json.loads(raw)
    if value['base'] != BASE or len(value['patches']) != 1:
...
def publish():
    data = json.loads((OUT / 'plan.json').read_text())
    subprocess.run(['git', 'diff', '--exit-code'], check=True)
    if git('write-tree') != data['trees'][1]:
    ...
    commits = []
    messages = ['feat(interviews): persist answer intent receipts for safe replay and reconnect']
    for index in (1,):
    ...
    subprocess.run(['git', 'archive', '--format=tar.gz', '--output=/tmp/copilot-stage/source-final.tar.gz', data['trees'][1]], check=True)
