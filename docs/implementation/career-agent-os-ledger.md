...
**历史记录，状态已更新：** 本节修改随后已恢复到重构分支，提交 `8d26fe2d`；
`56254871` / CI `35503935717` 完成新回归。后续pgvector切换见本节之后的记录；
不能继续把已经提交的代码称为“仅在恢复补丁”，也不能将旧评测数字外推到新功能。
没有修改 main、执行生产迁移或访问用户 Windows/GPU。
...
### 2026-09-20：P2本地推理首个生产接入切片

- **新增实际实现**：`app/local_inference` 独立 broker、私有 Unix socket、独立模型解释器、
  带老化的有限优先队列、驻留/空闲回收、先清理再释放预约、父服务死亡保护。
- **接入范围**：本地 Embedding 与 CrossEncoder 重排工厂现在共用 broker；不再由每个
  API/Celery 进程重复加载这两类权重。账户消费合同保留，异步 Embedding 使用真实异步 IPC。
- **明确边界**：只完成 P2 的 RAG 子集，不将 ASR/TTS/对齐/说话人/OCR计入接入完成。
  预约MiB不是实测峰值或OS显存沙箱；真实GPU和模型精度仍需后续实机。
- **版本/降级**：前缀、长度和适配合同进入向量代次；不复用旧代次冒充同一空间。
  已有一批重排执行后下一批拒绝，不释放整次消耗，不吞成普通fallback。
- **诊断**：`prepare_local_inference.py` 检查既有资产，拒绝覆盖现有配置；
  `run_local_inference.py` 前台运行或只读status，具体命令与限制见 deployment/local-first.md。
- **验证状态**：新增独立测试使用真实连接/进程及明确合成模型，覆盖排队取消、断连、
  日志洪泛、错误响应、服务强制退出、驻留回收和工厂接入。最终CI与计数在PR精确提交下记录。
  不继承旧提交绿灯，不将未运行的真实权重/麦克风测试记为通过。
- **仍未完成**：P2全部媒体模型接入、P4音频转写/角色完整纠正、P6实时语音、P7/P8其余
  用户旅程收敛、真实DeepSeek质量校准。原27项功能审查范围不缩减。


### 2026-09-20：P2 有限音频能力与资源故障边界

前置基线 `45ebb504` 已提交本地Embedding/重排独立服务，CI `35512610074` 成功；
本批在其上扩展，不重建第二套调度器。

- 新增四角色中的有限片段Qwen ASR与独立ForcedAligner：精确模型绑定、PCM包身份、
  输入/输出界限、原文字覆盖、合法时间或显式未对齐。模型仅由独立解释器加载。
- 通用文件转写与模拟录音回答已接入 `local_qwen_asr`，保留账户用量与未知结果保护。
  PCM解码真实流式背压，不把分块ASR误称连续流式语音；超限、末尾损坏、源变化不能
  成功返回不完整文字。清理失败不释放未知资源，过期排队任务不迟到执行。
- 旧RAG语义身份保持，CPU模型不按CUDA预约拒绝；新增配置只在明确传audio-python时
  检查既有Qwen资产，不下载、不覆盖配置、不强行混装ML环境。
- 工程回归含真实FFmpeg与IPC/子进程、API未知结果、队列过期和清理失败；模型SDK用
  明确替身测试，实际权重/性能/说话人未验收。最终提交结果以本次CI和JUnit为准。
- **仍未完成**：完整Qwen录音证据/角色纠正、TTS/说话人/OCR资源接入、vLLM流式ASR、
  WebRTC和打断播放、P7/P8剩余产品旅程、真实质量校准。本条不将这些工作计为完成。
