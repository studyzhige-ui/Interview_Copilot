...
> 一份让基础薄弱的开发者也能读懂这套系统是怎么工作、为什么这么设计的长篇文档。  
> 风格定调:按用户故事讲,不按文件夹讲;文件名只是地址注脚,故事才是主线。
...
**本章在讲什么**:在你开始一头扎进后面那些章节之前,先用几页篇幅把"这份文档是写给谁、不写给谁、怎么组织、推荐怎么读"这些元信息讲清楚。读完之后你应该能判断"我要不要继续读"以及"我应该从哪一章开始"。

### 0.1 这份文档是写给谁的

这份文档假设你**对 Interview Copilot 这套系统感兴趣,但是没看过它的源码,而且基础知识也不算扎实**。具体而言,如果你属于下面这几类人之一,这份文档就是写给你的:

- 你想搞清楚一套"AI 模拟面试系统"到底是怎么从零搭起来的——可能你自己也在准备求职;
- 你被指派去接手维护或者继续开发这个项目,但前任开发者已经离职、你只剩源码可看;
- 你想从这个项目里学一些可以抄到自己产品里的设计点(比如多厂商 LLM 路由怎么做、长期记忆怎么演化、Agent 怎么调工具),作为自己工作的参考;
- 你是一个在校学生或者刚入行的工程师,想看一套"中等规模、五脏俱全"的 LLM 应用长什么样子,作为学习材料。

这份文档**不是**写给已经非常熟悉本项目、只是想查某个具体 API 返回什么字段的高手——那种人直接读源码或者 OpenAPI 自动生成的文档更快。

关于"基础薄弱"这个定位的具体含义:我们假设你**听过但没深入用过**下面这些概念——大语言模型(LLM)、RAG(检索增强生成)、向量数据库、Embedding(嵌入)、Agent、JWT(JSON Web Token)、SSE(Server-Sent Events,服务端推送事件)、Celery(异步任务框架)、Fernet(对称加密)、SSRF(服务端请求伪造)、Prompt Cache(提示词缓存)。第 2 章会把这些概念一次讲清楚,后面章节再用到的时候直接引用。**如果你在哪一章遇到一个完全不认识的英文缩略词,先翻到第 2 章去查,基本都能找到**。

### 0.2 这份文档不写什么

为了避免你的期待落空,先把这份文档**不打算覆盖**的三类内容列清楚:

- **不带你逐个文件读代码**。这份文档不是源码导览。我们不会写"backend/app/api/ 这个文件夹里有 8 个文件,第一个文件是干什么的、第二个是干什么的"。文件名只在"如果你想看具体实现,可以打开这个文件"的注脚里出现,绝不作为讲解主线。讲解主线永远是"用户做了一件事,系统是怎么响应的"。
...
整份文档一共 18 章,根据章节性质分成**两种讲解模式**——读者要先理解这两种模式,才能在每章读到一半的时候知道"作者接下来要讲什么"。

#### 模式 A:四段式(用于"用户旅程"章节,第 4-12 章)

凡是讲清"用户做了一件具体的事,系统做了什么响应"的章节(比如注册、上传知识库、跟 AI 聊天、做模拟面试),都采用这种四段式结构。一章里依次是:

- **Part 1:这一章用到的主流技术——讲清原理**。先把这一章会用到的技术(比如做 RAG 要先讲清楚怎么切块、怎么算嵌入、向量库怎么选、ANN 算法是什么),按"先讲所有主流方案的工作原理"的方式扫一遍。**这一段只讲方案怎么工作,不评价好坏、不讲项目用了哪个**。读者读完这一段相当于上了一节"行业现状科普课"。
...
- **Part 3:设计取舍——每个决策为什么这么做不那么做**。把 Part 2 实现里的关键决策点拎出来,逐个回答"我们为什么选 X 不选 Y"。比如"为什么用预签名 URL 不用后端中转""为什么用 BGE-M3 不用 OpenAI 嵌入"。这一段是给"想抄设计、想理解动机"的读者看的。
...
不是单一用户旅程、而是系统总览或者横切主题的章节,不用四段式——但**仍然要求详细易懂,不允许走简化路线**。比如第 3 章"系统全景"要讲清所有组件、第 13 章"安全防线"要把分散在各处的安全设计集中讲清楚、第 14 章"数据住在哪"要把每张表每个 Redis key 每个 Milvus collection 的用途列全。这类章节按主题划分小节,每个小节都用流程图 + 中文逐框讲解的方式展开。
...
| 14 | 数据住在哪 | 详细讲解 | Postgres 13 张表、Redis 六种用途、Milvus 两个 collection、MinIO 桶设计 |
...
- **复杂流程都画图**:每个章节的关键流程都会配一张架构图或时序图(Mermaid 格式)。**每张图后面都跟着一段"逐框逐线讲解"**——讲清图里每个方框、每条箭头代表什么、为什么这样连接。我们不让图自己说话,文字解释永远不可省略。
...
下面 1.2 节就用画面感叙事的方式,过一遍这四个目标在产品里具体长什么样。

### 1.2 用户能用它做的五件事

下面每一小节先用"小王做了什么"的故事讲清楚用户在屏幕上看到了什么、做了什么操作、得到了什么反馈。然后用一段"后台干什么"概述背后用到了哪些技术(对应的详细章节会指明)。基础薄弱的读者**第一遍看这五件事的时候**,只需要建立直觉——"哦,原来这个项目能干这五件事"——不必试图理解技术名词。
...
**本章在讲什么**:这一章是为基础薄弱的读者准备的"前置知识扫盲"——把后面所有用户旅程章节里会反复出现的技术概念一次性讲清楚,让你读到第 4-12 章的时候不会被"JWT""SSE""Celery""Fernet""SSRF""Prompt Cache""ReAct""Embedding""BM25""重排器"这些英文术语挡住。每个概念都按"是什么 → 怎么工作 → 本项目用在哪一章"的格式介绍,**专注于建立直觉**,不深入细节(细节在后续章节里讲)。
...
总数清点:基础设施 7 个(PostgreSQL + Redis + 应用 MinIO + MinIO 创建桶辅助 + Milvus standalone + Milvus etcd + Milvus minio)+ 应用进程 5 个(api + frontend + worker-transcription + worker-light + beat)+ dev 专用 nginx 1 个 = 最多 **12 个容器**。
...
> 本章组织方式:**先教课、再讲项目、再讲取舍、最后实战 QA**。基础薄弱的读者建议顺读;熟手可以跳着看 Part 2(项目实现)和 Part 4(面试 QA)。
...
本项目支持**多 backend 注册**——通过配置(`ASR_PROVIDER` 环境变量)在运行时切换。具体在 1.4 节 + Part 2 展开。
...
本项目支持 **4 个 ASR backend**,通过 `ASR_PROVIDER` 环境变量切换:
...
**本章在讲什么**:用户在 Interview Copilot 用每一个功能都涉及"数据被读写到某处"——他注册时写一行 `users` 表、上传简历时写 `user_uploads`、聊天时写 `chat_messages`、面试结束时写 `interview_records`、Memory v3 演化时写 `knowledge_docs`……这一章把项目所有的数据存储**集中讲清楚**——Postgres 13 张表、Redis 6 种用途、Milvus 2 个 collection、应用层 MinIO 各存什么——并给出完整 ER 图,让读者能在脑子里精确画出"用户做某操作时数据写到哪几个位置"。

> **本章读法**:可以按需查询——遇到具体业务章节里"写哪张表"的疑问就翻到本章对照。也可以一次性顺读建立完整数据视图。

### 14.1 业务对象 → Postgres 表的映射

整套系统的关系数据共 **13 张 Postgres 表**,按业务模块分 5 组:
...
`memory_audit_log` 上面已经讲过——本节只列出来确认它是属于"Memory v3 这组"的。
...
同样的模式:`OPENAI_API_KEY` / `OPENAI_API_BASE`、`ANTHROPIC_API_KEY` / `ANTHROPIC_API_BASE`、`QWEN_API_KEY`(DashScope)/ `DASHSCOPE_API_BASE`、`ZHIPU_API_KEY`(智谱 / z.ai)、`MOONSHOT_API_KEY`、`XIAOMI_API_KEY`、`NVIDIA_NIM_API_KEY`、`GEMINI_API_KEY`、`SILICONFLOW_API_KEY`,等等。
...
| `ASR_PROVIDER` | local_whisperx | ASR 提供方:local_whisperx / openai / siliconflow / dashscope |
| `WHISPER_MODEL` | Systran/faster-whisper-large-v3 | Whisper 模型 |
...
| `MILVUS_RESUME_COLLECTION` | interview_copilot_resume | 简历 collection |
| `MILVUS_DENSE_INDEX_TYPE` | HNSW | 索引算法 |
| `MILVUS_SIMILARITY_METRIC` | IP | 距离度量 |
| `RAG_VECTOR_TOP_K` | 8 | 向量检索 top-K |
| `RAG_FUSION_TOP_K` | 6 | RRF 融合后 top-K |
| `RAG_RERANK_TOP_N` | 5 | 重排器最终 top-N |
...
恭喜你读到这里——这份 **17 章近 20 万字的项目深度文档**到这里就结束了。从第 0 章讲文档结构,到第 16 章把所有概念串起来,再到这一章作为速查附录,你应该已经能够:
...
如果你是想抄某些设计点(多厂商 LLM 路由 / Memory v3 / Runtime Director 状态机 / Agent 三层落盘等)的开发者——希望本文档讲清了"为什么这么设计"+"实现细节怎么处理"——你可以基于这些直接复用到自己的产品里。
