...
## 五、用户决策记录（2026-07-06 已确认）

1. **分期顺序**：按方案顺序推进（Phase 0 → 8）。
2. **MOCK-7 语音链路**：**接通**——前端转写后上传音频 blob，复盘可回放原声，为「表达流畅度评估」保留地基（Phase 5 执行，成本 M）。
3. **AGT-1 turn 解耦（6b）**：Phase 0 先做最小修复（用户消息先落库 + finally 保底）；完整方案 **Phase 7 时立设计文档**，与 agent 上下文管理计划合流。
