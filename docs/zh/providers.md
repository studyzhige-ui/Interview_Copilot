...
| 角色 | 配置 env 变量 | 注册表文件 |
|---|---|---|
| **LLM**（聊天 / Agent / 模拟面试官） | 在「模型」页用户级选择 | `backend/app/core/model_catalog.py` + `user_model_selection.py` + `llm_client_factory.py` |
...
要 100% 离线还得加一个本地 LLM（Ollama / vLLM）— 在 `app/services/model_sources/providers.py` 加一条 `default_api_base="http://localhost:11434/v1"`，再到 `model_sources/vendors/<id>.py` 放一个对应的适配器 SPEC。
...
LLM 是**用户级**配置（在「模型」页选）—— 同一部署里不同用户可以用
不同模型。运行时 profile 目录由每家厂商 `/v1/models` 实时驱动，再与
provider 默认连接信息 join —— DeepSeek、OpenAI、Anthropic、Gemini、通义、
Moonshot、智谱、MiMo、NVIDIA。新用户能看到全部 profile，只配自己实际用
的 API key。

相关三个模块（P8-10 时从原 737 行 `model_registry.py` 拆出，原文件保留
为 70 行兼容垫片）：

- `backend/app/core/model_catalog.py` —— 厂商驱动的目录 + Redis 缓存；
  `ModelProfile`、`ROLE_DEFAULTS`、`get_profile`
- `backend/app/core/user_model_selection.py` —— 每用户 角色→profile 落盘
  （`users.model_selection_json`）
- `backend/app/core/llm_client_factory.py` —— api-key / override 解析、
  `AsyncOpenAI` + LlamaIndex 客户端缓存、`_build_llm_instance`、
...
所有支持的 LLM 厂商都走**同一个** `OpenAILike` 客户端。每家
provider 的 `default_api_base`（在 `model_sources/providers.py` 里
声明）加上用户的 API key，runtime 就能调起来：
...
切换 provider 完全就是 **(api_base, api_key, model_id)** 三元组的事 ——
runtime 代码完全无分支。加新厂商 = `providers.py` 加一行 +
`model_sources/vendors/<id>.py` 加一个适配器 SPEC。

`backend/app/core/llm_client_factory.py:_build_llm_instance` 实际就 6 行：
...
1. 每家厂商都有一份声明式适配器 SPEC，位于 `model_sources/vendors/<id>.py`（API 路径 / 鉴权方式 / 响应字段 / chat-only 过滤器）
2. Pipeline 并发 fan-out 到所有 SPEC，过滤后送给 `curated.py` UX 层做"显示名 / 排序 / 隐藏变体"美化
3. 结果写 Redis `model_catalog:v5:<provider>`（24h TTL）+ 一份无 TTL 的 last-known-good 快照。某家厂商挂掉只回退该家的 LKG，其他家不受影响
4. 仓库还附带一份 `seed_catalog.json` 静态快照，让 fresh clone 在没刷新过的状态下也能看到 polished 列表

**手动触发刷新**：

- **Web UI**：模型页 → 点 **「刷新模型库」** 按钮（在 "Ping 测试" 旁）。重新调每家厂商 `/v1/models` 重写缓存
...
- **CLI**（适合 cron / CI 预热）：
  ...
  python scripts/refresh_models.py --write-seed    # 重新生成仓库内置 seed
  ```

**自动刷新**：Celery beat 任务 `tasks.refresh_model_catalog` 每天 04:00 Asia/Shanghai 自动跑一次，保持缓存常新。

**支持的厂商**：9 家直接开箱可用（详见上表）。加新厂商 = `providers.py` 一行 + 一个适配器 SPEC，runtime 路径不变。

**Chat-only 过滤**：每家适配器声明自己的过滤规则，丢掉厂商一起返回的 embedding / 图像 / 音频 / TTS / 安全分类器等非聊天模型。参考 `vendors/openai.py::_chat_filter`，OpenAI 的过滤覆盖 `embed`、`whisper`、`tts-`、`dall-e`、`realtime` 等关键字。
