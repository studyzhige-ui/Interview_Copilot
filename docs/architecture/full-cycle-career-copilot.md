...
> 状态：`Superseded`（自 2026-08-26 起不再具有现行产品或目标架构规范效力）
> 取代者：[`career-agent-os-blueprint.md`](./career-agent-os-blueprint.md)
> 保留用途：历史产品与架构基线、现有代码设计理由和迁移取证；不得用于覆盖当前已批准蓝图、正式 Contract 或已登记架构决策。
> 阅读规则：本文后续所有“唯一现行”“必须遵守本文”等表述均只反映 2026-08-13 的历史基线，不再是当前规范声明。

## 0. 文档契约与局部覆盖规则

### 0.1 历史文档地位

本文曾是 2026-08-13 至 2026-08-25 期间的产品与目标架构基线。自 2026-08-26 起，当前唯一现行产品与目标架构由 [`career-agent-os-blueprint.md`](./career-agent-os-blueprint.md) 定义；本文只作为 Historical / Superseded 资料保留。
