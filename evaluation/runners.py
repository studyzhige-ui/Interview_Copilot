...
def load_dataset(
    ...
    if not dataset_path.exists():
        raise FileNotFoundError(
            f"Evaluation dataset missing: {dataset_path}. Copy "
            "evaluation/golden_dataset.example.jsonl and curate project-owned rows."
        )
...
async def run_generation(rows: list[dict[str, Any]]) -> dict[str, Any]:
    """End-to-end RAG quality: retrieve → evaluator answer → RAGAS scores.
    ...
    summary["hallucination_gate_rate"] = round(
        sum(1 for d in details if not d["retrieval_hit"]) / len(details),
        4,
    )
...
async def _stream_with_ttfb(llm, prompt: str) -> tuple[str, float]:
    ...
    (for example, when a provider drops a connection).
    """
    t_start = time.perf_counter()
    ttfb_ms = 0.0
    ttfb_recorded = False
